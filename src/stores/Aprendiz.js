...
export const useAprendizStore = defineStore("aprendiz", () => {
  ...
  const listarAprendices = async () => {
    try {
      const r = await axios.get("http://localhost:5037/api/Aprendices/Listar", {
  ...
  const listarXId = async (id) => {
    try {
      const r = await axios.get(`http://localhost:5037/api/Aprendices/listar2/${id}`, {
        headers: { "x-token": useUsuarios.xtoken },
      });
      console.log(r);
      return r;
  ...
  const activarDesactivar = async (id) => {
    try {
      const r = await axios.put(
        `http://localhost:5037/api/Aprendices/activarDesactivar/${id}`,
        {},
        { headers: { "x-token": useUsuarios.xtoken } }
      );
  ...
  const guardarAprendiz = async (num, doc, nom, ema, tel, firmaFile) => {
    try {
      ...
      const r = await axios.post(
        "http://localhost:5037/api/Aprendices/insertaraprendiz",
        {
          IdFicha: num,
          cc: doc,  
          nombre: nom,
  ...
  const editarAprendiz = async (id, num, doc, nom, ema, tel, firmaFile) => {
    try {
      ...
      const r = await axios.put(
        `http://localhost:5037/api/Aprendices/editarAprendiz/${id}`,
        {
          IdFicha: num,
          cc: doc,
          nombre: nom,
  ...
  const eliminar = async (id) => {
    try {
      const r = await axios.delete(`http://localhost:5037/api/Aprendices/Eliminar/${id}`, {
        headers: { "x-token": useUsuarios.xtoken },
      });
      console.log(r);
      Notify.create({
  ...
  const obtenerFichas = async () => {
    try {
      const r = await axios.get("http://localhost:5037/api/Fichas/listar");
