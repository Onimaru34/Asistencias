...
export const useBitacoraStore = defineStore("bitacora", () => {
  ...
  const listarBitacorasXFecha = async (inicio, fin) => {
    try {
      ...
      let url = `http://localhost:5037/api/Bitacoras/bitacora/${inicio}`;
      if (fin) {
        url += `/${fin}`;
      }
...
  };
  ...
  const listarXIdFicha = async (IdFicha, fechaInicio, fechaFin) => {
    try {
        ...
        let r = await axios.get(`http://localhost:5037/api/Bitacoras/listarBitacoraIdFicha/${IdFicha}/${fechaInicio}/${fechaFin}`,
          {
            headers: {
              "x-token": useUsuarios.xtoken,
            },
  ...
  const crearRegistroBitacora = async (cedula) => {
    try {
      const response = await axios.post('http://localhost:5037/api/Bitacoras/insertar2', { cc: cedula },
        {
          headers: {
            "x-token": useUsuarios.xtoken,
          },
