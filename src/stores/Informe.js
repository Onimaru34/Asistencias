...
const useUsuarios = useUsuariosStore()
  ...
  const listarTodasLasBitacoras = async () => {
    try {
      let r = await axios.get('http://localhost:5037/api/Bitacoras/listar',
        {
          headers: {
  ...
  const listarBitacorasPorFichaYFecha = async (IdFicha, fecha) => {
    ...
    try {
      let r = await axios.get(`http://localhost:5037/api/Bitacoras/ListarBitacoraIdFicha/${IdFicha}/${fecha}/${fecha}`,
        {
          headers: {
            "x-token": useUsuarios.xtoken,
          },
  ...
  const obtenerFichas = async () => {
    try {
      const response = await axios.get('http://localhost:5037/api/Fichas/listar',
        {
          headers: {
            "x-token": useUsuarios.xtoken,
          },
