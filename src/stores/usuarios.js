...
export const useUsuariosStore = defineStore("usuario", () => {
  ...
  const activarDesactivarUsuarios = async (id) => {
    try {
      let r = await axios.put(`http://localhost:5037/api/Usuarios/activarDesactivar/${id}`);
  ...
  const sendPasswordResetLink = async (email) => {
    try {
      const response = await axios.post("http://localhost:5037/api/Usuarios/recover-password", { email });
      Notify.create({
        color: "positive",
